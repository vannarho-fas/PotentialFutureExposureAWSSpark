...
2. Since writing this I have experimented with the design further to write out to Cassandra (see the files pfe_scenarios.py and spark-submit-cassandra.sh). This can be used as-is on local by setting up a local Cassandra node and by adding some additional parameters to the spark-submit job (to pick up the cassandra drivers). Using it on AWS requires some additional credentials and set up of AWS keyspaces. I haven't documented either instructions here. 

...
def rdd_load_cassandra_write_csv(mc_scenarios_rdd, output_dir, counterparty):
    ...
    df_agg = sqlContext.createDataFrame([], schema)

    for row_num in range(mc_scenarios_rdd.count()):
...
if __name__ == "__main__":
    ...
    args = {'NSim': sys.argv[1],
            'NPartitions': sys.argv[2],
            'a': sys.argv[3],
            'sigma': sys.argv[4],
            'usd_swap_curve_file': sys.argv[5],
